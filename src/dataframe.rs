...
pub struct GroupedDataFrame<'a, D: Clone + DeserializeOwned + Serialize, I: Eq + Hash + Clone, F>
where
    F: Fn(&D) -> I,
{
    dataframe: &'a DataFrame<D>,
...
impl<D: Clone + DeserializeOwned + Serialize> DataFrame<D> {
    ...
    pub fn sort<F>(&self, comp: F) -> Self
    where
        F: Fn(&D, &D) -> bool,
    {
        let mut new_data = Vec::<D>::new();
    ...
    /// ).head(10))
    /// ```
    pub fn groupby<F, I: Eq + Hash + Clone>(&self, index: F) -> GroupedDataFrame<'_, D, I, F>
    where
        F: Fn(&D) -> I,
    {
    ...
    pub fn find<F>(&self, condition: F) -> Option<&D>
    where
        F: Fn(&D) -> bool,
    {
    ...
    pub fn split(&self, frac: f64) -> (Self, Self) {
        ...
        let data2: Vec<D> = new_data[amount..self.len()]
            .iter()
            .map(|x| x.clone())
            .collect();
    ...
    pub fn to_parquet(&self, path: String) -> Result<(), errors::Error> {
        ...
        match write_chunk_parquet(&path, Schema::from(schema), Chunk::new(arrays)) {
            ...
            Err(_) => Err(errors::Error {
                message: "Error saving parquet!".to_string(),
            }),
        }
    }
}
...
impl<'a, D: Clone + DeserializeOwned + Serialize> SliceDataFrame<'a, D> {
    fn new(dataframe: &'a DataFrame<D>, start: usize, end: usize) -> Self {
        log::trace!(
            "Creating a new SliceDataFrame with start: {} and end: {}",
            start,
            end
        );
        SliceDataFrame {
    ...
    pub fn clone(&self) -> DataFrame<D> {
        ...
        let data = self.dataframe.data[self.start..self.end]
            .iter()
            .map(|x| x.clone())
            .collect();
        DataFrame::new(data)
    ...
    pub fn take(&self, num: usize) -> Vec<D> {
        let min_num = std::cmp::min(num, self.len());
        let data = self.dataframe.data[self.start..(self.start + min_num)]
            .iter()
            .map(|x| x.clone())
            .collect();
        data
...
impl<'a, D: Clone + DeserializeOwned + Serialize, I: Eq + Clone + Hash, F>
    GroupedDataFrame<'a, D, I, F>
where
    F: Fn(&D) -> I,
{
    ...
    pub fn agg<S: Clone + DeserializeOwned + Serialize, G>(&self, aggregator: G) -> DataFrame<S>
    where
        G: Fn(&I, &Group<D>) -> S,
    {
        ...
        let data = hashmap
            .iter()
            .map(|(ind, group)| aggregator(ind, &Group { data: group.iter() }))
            .collect();
        DataFrame::new(data)
    }
}
