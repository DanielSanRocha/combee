...
fn test_read_basic_parquet() {
    ...
    assert_eq!(*rowd, Data { name: String::from("Daniel"), age: 26 });

    let rows = df.find(|x| x.name == "Sergio").unwrap();
    assert_eq!(*rows, Data { name: String::from("Sergio"), age: 30 });

    let rowl = df.find(|x| x.name == "Leticia").unwrap();
    assert_eq!(*rowl, Data { name: String::from("Leticia"), age: 22 });
}

#[test]
fn test_read_complex_parquet() {
    let df = combee::read_parquet::<Complex>(String::from("tests/fixtures/complex.parquet")).unwrap();
