...
fn test_basic_csv_apply() {
    ...
    let df_message = df.apply(move |row| Message { message: format!("Hello {} with {} years!", row.name, row.age)});

    assert_eq!(df_message.len(), 3);

    let messages = df_message.take(3);
    assert_eq!(*messages.index(0), Message { message: String::from("Hello Daniel with 26 years!") });
    assert_eq!(*messages.index(1), Message { message: String::from("Hello Sergio with 30 years!") });
    assert_eq!(*messages.index(2), Message { message: String::from("Hello Leticia with 22 years!") });
}
